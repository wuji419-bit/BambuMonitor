...
- 登录后可在“设置 > 拓竹账号”继续添加账号。账号备注为可选项，可跳过或稍后修改；未填写时界面显示脱敏账号。
- 多个账号中的设备按序列号自动合并，同一台打印机只保留一张设备卡、一个 MQTT 状态连接和一个上游摄像头源。
- 每个账号可以单独同步、重新登录和移除；单个账号失效不会中断其他账号。删除最后一个账号会清除浏览器登录会话并返回登录页。
- 从单账号旧版本升级时会自动迁移原有会话，不需要重新登录。升级前仍建议完整备份 `data` 目录。
...
`./data` 包含服务配置、多账号会话和加密材料。`secret.key` 丢失或损坏会使 `session.enc` 无法解密，此时需要重新登录全部账号。必须备份整个 `data` 目录，使密钥与其加密的数据保持同一版本。
...
先在“设置 > 拓竹账号”查看是否只有一个账号失效。单个账号可直接使用“重新登录”恢复，不会影响其他账号。若全部账号都无法恢复，再确认 `./data` 被正确挂载且没有被临时目录替代。若 `secret.key` 与 `session.enc` 不匹配、丢失或损坏，只能清理失效会话并重新登录，再从完整且匹配的 `data` 备份恢复其他数据。
