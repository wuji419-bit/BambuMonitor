...
当前实时进度来自打印机本地 MQTT 服务，应用需要能从当前电脑访问到打印机的 `8883` 端口。人在外面时，普通公网环境不能直接连家里/工作室的局域网打印机；建议先用 Tailscale、ZeroTier、WireGuard 或路由器 VPN 连回同一网络，再在应用里填写这个隧道里可访问的打印机 IP。

登录拓竹账号只用于同步绑定设备和访问码，不等于接入拓竹官方 App 的远程实时通道。
...
后续只有应用代码、图标或安装包内容发生实际变化时，才递增一个小版号并重新打包，例如：`1.0.3` -> `1.0.4`。
