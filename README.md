...
- **一个窗口管理多个账号**：账号备注可跳过；同一序列号的设备会自动合并，不会重复建立状态或摄像头连接。
...
- 在“设置 > 拓竹账号”中可以继续添加账号；账号备注为可选项，可以直接跳过。
- 设备名称后会显示账号备注；未填写备注时显示脱敏后的账号。若同一设备属于多个已登录账号，会合并显示这些账号标签。
- 设备以序列号去重，同一台打印机只显示一张卡片，并只保留一条状态连接和一个摄像头源。
- 每个账号可以单独同步、修改备注、重新登录或移除。某个账号失效不会让其他账号退出；删除最后一个账号会退出 BambuMonitor。
- 从旧版本升级时会自动迁移原有登录状态，无需为了启用多账号重新登录。
...
- 桌面版登录会话通过 Electron 系统安全存储加密保存；NAS 版使用本地密钥加密保存在挂载的数据目录中。
- 渲染界面和浏览器只接收脱敏账号信息；访问令牌与打印机连接凭据由桌面主进程或 NAS 服务端持有。
- 密码、验证码和访问令牌不会写入项目仓库。
