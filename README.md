...
- 摄像头墙：有本地 IP 和访问码时会尝试打开本地摄像头画面，并可为每台打印机填写自定义 MJPEG / 快照 URL
...
本项目不提供、也不会声称支持拓竹官方 App 那种公网远程实时监控。登录拓竹账号只用于同步绑定设备、访问码和云端概览状态；它不代表本应用接入了拓竹官方远程实时通道。
...
当前完整实时进度来自打印机本地 MQTT 服务，应用需要能从当前电脑访问到打印机的 `8883` 端口。人在外面时，普通公网环境不能直接连家里/工作室的局域网打印机；应用会先进入“云端概览模式”，只显示云端在线/粗略打印状态，并提示该模式不包含温度、AMS、层数和精确实时进度。
...
自动摄像头模式同样依赖局域网或 VPN。H2D / X1 / P2S 等机型会优先通过本机 `ffmpeg` 将 RTSPS 画面转换为浏览器可显示的 MJPEG；A1 / A1 mini / P1 / A2 系列会尝试本地 `6000` 端口 JPEG 摄像头协议，并在界面中用快照帧刷新，减少 Electron 里 MJPEG 长连接卡住的问题。如果某台设备仍被官方软件占用或网络不可达，可以在设置里为对应打印机填写外部 MJPEG 或快照 URL。
